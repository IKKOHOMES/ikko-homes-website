...
export function PaymentPlanEditor({ quoteTotal, instalments, onSave, onSync }: {
  ...
  const updatePercent = (index: number, percentage: number) => { try { setLines((current) => updateSchedulePercent(current, index, percentage, quoteTotal, lastEditableIndex(current))); setError(''); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to update instalment.'); } };
  const updateAmount = (index: number, amount: number) => { try { setLines((current) => updateScheduleAmount(current, index, amount, quoteTotal, lastEditableIndex(current))); setError(''); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to update instalment.'); } };
