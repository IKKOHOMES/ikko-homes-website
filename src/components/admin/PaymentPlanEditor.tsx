...
export function PaymentPlanEditor({ quoteTotal, instalments, onSave, onGenerate }: {
  ...
  const updatePercent = (index: number, percentage: number) => { try { setLines((current) => updateSchedulePercent(current, index, percentage, quoteTotal)); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to update instalment.'); } };
  const updateAmount = (index: number, amount: number) => { try { setLines((current) => updateScheduleAmount(current, index, amount, quoteTotal)); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to update instalment.'); } };
