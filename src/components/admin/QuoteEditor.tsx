...
export function QuoteEditor({ quote, onSave, onConfirm, documentActions }: {
  ...
  return <form className="quote-editor" onSubmit={(event) => void save(event)}>
    ...
    <div className="quote-editor__lines"><div className="quote-editor__line-header"><span>No.</span><span>Item &amp; description</span><span>Quantity</span><span>Rate</span><span>Amount</span><span aria-hidden="true" /></div>{lines.map((line, index) => <fieldset className="quote-editor__line" key={line.id ?? `${index}-${line.displayName}`}>
      ...
      <label>Item &amp; description<input aria-label={`Line ${index + 1} description`} value={line.displayName} onChange={(event) => updateLine(index, { displayName: event.target.value })} /></label>
      <label>Quantity<input aria-label={`Line ${index + 1} quantity`} min="1" type="number" value={line.quantity} onChange={(event) => updateLine(index, { quantity: Number(event.target.value) })} /></label>
      <label>Rate<input aria-label={`Line ${index + 1} rate`} disabled={line.isTbd} min="0" step="0.01" type="number" value={line.isTbd ? '' : line.unitPrice} onChange={(event) => updateLine(index, { unitPrice: Number(event.target.value) })} /></label>
      ...
      <div className="quote-editor__line-actions"><label className="quote-editor__tbd"><input aria-label={`Line ${index + 1} T.B.D.`} checked={line.isTbd} type="checkbox" onChange={(event) => updateLine(index, { isTbd: event.target.checked, unitPrice: event.target.checked ? 0 : line.unitPrice })} /> T.B.D.</label><button className="admin-text-button admin-text-button--danger" type="button" onClick={() => setLines((current) => current.filter((_, currentIndex) => currentIndex !== index))}>Remove</button></div>
    </fieldset>)}</div>
