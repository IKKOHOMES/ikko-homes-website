...
function AppRoutes() {
  ...
  return <div className={`app-shell${isAdminRoute ? ' app-shell--admin' : ''}`}>
    ...
    <main className="page-shell"><Routes>
        ...
        <Route path="/cart" element={<Navigate replace to="/order" />} /><Route path="/order" element={<CartPage />} /><Route path="/confirmation" element={<Confirmation />} />
...
function CartPage() { const { lines, removeLine, clear } = useCart(); const { user } = useCustomerAuth(); const navigate = useNavigate(); const hasCabinetry = lines.some((line) => line.kind === 'cabinetry'); const cabinetryNames = lines.filter((line) => line.kind === 'cabinetry').map((line) => line.name).join(', '); const [form, setForm] = useState<CustomerDetails>({ firstName:'',lastName:'',email:'',phone:'',address:'',note:'' }); useEffect(() => {
  ...
  prefill({
    firstName: stringValue(user.user_metadata.first_name),
    lastName: stringValue(user.user_metadata.last_name),
    email: user.email ?? '',
    phone: stringValue(user.user_metadata.phone),
  });
  void getCustomerSupabaseClient().from('customers').select('first_name,last_name,email,phone,address').eq('auth_user_id', user.id).maybeSingle().then(({ data }) => {
    if (data) prefill({ firstName: data.first_name, lastName: data.last_name, email: data.email, phone: data.phone, address: data.address });
  });
  return () => { active = false; };
}, [user]); const [error, setError] = useState(''); const [submitting, setSubmitting] = useState(false); const subtotal = lines.reduce((total,line) => total + (line.kind === 'furniture' ? line.price * line.quantity : 0),0); if (!lines.length) return <Editorial title="Your cart is waiting." copy="Explore furniture or upload a cabinetry drawing to begin." />; const submit = async () => { if (!form.firstName || !form.lastName || !form.email || !form.phone || !form.address) { setError('Please complete your contact and project details.'); return; } setSubmitting(true); setError(''); try { const order = await submitOrder(lines, form); clear(); navigate('/confirmation', { state: order }); } catch (submissionError) { setError(submissionError instanceof Error ? submissionError.message : 'We could not submit your order.'); } finally { setSubmitting(false); } }; return <section className="cart-page"><div><p className="eyebrow">Your order</p><h1>Review your order.</h1><div className="cart-lines">{lines.map((line) => <article className="cart-line" key={line.id}><div className={`mini-image product-image--${line.imageTone}`} /><div><h3>{line.name}</h3><p>{line.kind === 'cabinetry' ? `Drawing attached: ${line.upload.name}` : line.finish}</p></div><b>{line.kind === 'cabinetry' ? 'T.B.D.' : `$${line.price.toLocaleString('en-AU')}.00`}</b><button onClick={() => removeLine(line.id)}>Remove</button></article>)}</div><h2>Your details</h2><div className="form-grid">{(['firstName','lastName','email','phone','address'] as const).map((field) => <input key={field} placeholder={field === 'firstName' ? 'First name' : field === 'lastName' ? 'Last name' : field === 'email' ? 'Email address' : field === 'phone' ? 'Phone number' : 'Delivery / project address'} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} />)}<textarea placeholder="Order notes (optional)" value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></div></div><aside className="summary"><h2>Order summary</h2><p>Furniture <b>${subtotal.toLocaleString('en-AU')}.00</b></p>{hasCabinetry && <p>{cabinetryNames} <b className="orange">T.B.D.</b></p>}<hr /><p className="due">Amount due today <b>{hasCabinetry ? '$0.00' : `$${subtotal.toLocaleString('en-AU')}.00`}</b></p>{hasCabinetry && <small>We’ll prepare a quotation after reviewing your cabinetry drawing.</small>}{error && <p className="error" role="alert">{error}</p>}<button className="button" disabled={submitting} onClick={() => void submit()}>{submitting ? 'Submitting…' : 'Confirm order & request quotation'}</button></aside></section> }
function Confirmation() { const { state } = useLocation() as { state?: { orderNumber?: string; documentKind?: 'quote-pending' } }; return <Editorial title="Thank you for your order." copy={state?.orderNumber ? `${state.orderNumber} has been received. Quote v1 is being prepared for your review.` : 'Your confirmation has been prepared. Check your inbox for the next step.'} /> }
