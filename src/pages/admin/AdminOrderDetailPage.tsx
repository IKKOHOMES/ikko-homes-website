...
export function AdminOrderDetailPage() {
  ...
  return <section className="admin-dashboard admin-order-detail">
    ...
    <div className="admin-order-detail__documents"><section className="admin-detail-section"><h2>Order lines</h2>{detail.lines.map((line) => <div className="admin-line" key={line.id}><span>{line.name}{line.finish ? ` · ${line.finish}` : ''}</span><span>× {line.quantity}</span><b>{line.unitPrice === null ? 'T.B.D.' : `$${line.unitPrice.toLocaleString('en-AU', { minimumFractionDigits: 2 })}`}</b></div>)}</section>
