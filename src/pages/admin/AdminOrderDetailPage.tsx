...
import { confirmQuote, getAdminOrder, saveQuote, type AdminOrderDetail } from '../../lib/admin-api';
...
export function AdminOrderDetailPage() {
  ...
  return <section className="admin-dashboard admin-order-detail"><Link className="admin-back" to="/admin/orders">← Back to orders</Link><div className="admin-page-heading"><div><p className="eyebrow">Order {detail.order.number}</p><h1>{detail.order.customerName}</h1></div><StatusBadge status={detail.order.status} /></div>
    ...
    {latestQuote ? <QuoteEditor quote={latestQuote} onSave={async (input) => { await saveQuote(input); load(); }} onConfirm={async (quoteId) => { await confirmQuote(detail.order.id, quoteId); load(); }} /> : <p className="admin-empty">Quote v1 is being prepared.</p>}
  </section>;
}
