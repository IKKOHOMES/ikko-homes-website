...
export function AdminOrderDetailPage() {
  ...
  return <section className="admin-dashboard admin-order-detail"><Link className="admin-back" to="/admin/orders">← Back to orders</Link><div className="admin-page-heading"><div><p className="eyebrow">Order {detail.order.number}</p><h1>{detail.order.customerName}</h1></div><StatusBadge status={detail.order.status} /></div>
    ...
    {latestQuote ? <><QuoteEditor quote={latestQuote} onSave={async (input) => { await saveQuote(input); load(); }} onConfirm={async (quoteId) => { await confirmQuote(detail.order.id, quoteId); load(); }} />{latestQuote.status === 'confirmed' && <PaymentPlanEditor quoteTotal={latestQuote.total} instalments={detail.paymentPlan} onSave={async (instalments) => { await savePaymentPlan(detail.order.id, latestQuote.id, instalments); load(); }} onGenerate={generateInvoices} />}</> : <p className="admin-empty">Quote v1 is being prepared.</p>}
    {detail.invoices.length > 0 && <section className="admin-detail-section"><h2>Instalment invoices</h2>{detail.invoices.map((invoice) => <div className="admin-line" key={invoice.id}><span>{invoice.number}{invoice.dueOn ? ` · due ${invoice.dueOn}` : ''}</span><span className="status-badge">{invoice.status}</span><b>${invoice.total.toLocaleString('en-AU', { minimumFractionDigits: 2 })}</b>{invoice.status === 'issued' && <button className="admin-secondary-button" disabled={working} onClick={() => void markPaid(invoice.id)} type="button">Mark paid</button>}</div>)}{invoiceError && <p className="error" role="alert">{invoiceError}</p>}</section>}
  </section>;
}
