...
export function AdminOrderDetailPage() {
  ...
  return <section className="admin-dashboard admin-order-detail">
    ...
    <div className="admin-order-detail__documents"><section className="admin-detail-section admin-order-detail__order-lines"><h2>Order lines</h2><div className="admin-order-lines__header"><span>Item</span><span>Colour</span><span>Size</span><span>Quantity</span><span>Cost</span></div>{detail.lines.map((line) => <div className="admin-line admin-order-line" key={line.id}><span>{line.name}</span><span>{line.finish ?? '—'}</span><span>—</span><span>× {line.quantity}</span><b>{line.unitPrice === null ? 'T.B.D.' : `$${line.unitPrice.toLocaleString('en-AU', { minimumFractionDigits: 2 })}`}</b></div>)}</section>
    {detail.drawings.length > 0 && <section className="admin-detail-section"><h2>Attached plans</h2>{detail.drawings.map((drawing) => drawing.signedUrl ? <a className="admin-download" href={drawing.signedUrl} key={drawing.fileName} rel="noreferrer" target="_blank">Download {drawing.fileName}</a> : <p key={drawing.fileName}>{drawing.fileName} (link unavailable)</p>)}</section>}
    </div>
    {latestQuote ? <><div className="admin-quote-actions"><DocumentActions disabled={latestQuote.status !== 'confirmed'} documentId={latestQuote.id} documentType="quote" recipientEmail={detail.customer.email} /></div><QuoteEditor quote={latestQuote} onSave={async (input) => { await saveQuote(input); load(); }} onConfirm={async (quoteId) => { await confirmQuote(detail.order.id, quoteId); load(); }} />{latestQuote.status === 'confirmed' && <PaymentPlanEditor quoteTotal={latestQuote.total} instalments={detail.paymentPlan} onSave={async (instalments) => { const persisted = await savePaymentPlan(detail.order.id, latestQuote.id, instalments); void load(); return persisted; }} onSync={syncInvoiceDrafts} />}</> : <p className="admin-empty">Quote v1 is being prepared.</p>}
